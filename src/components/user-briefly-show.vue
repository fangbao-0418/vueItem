  ...
  <div>
    <router-link :to="{name: 'person'}" class="user bg-color-white" tag="div" v-if="loginStatus">
      ...
      <div v-if="userInfo.restAward > 0" class="user-remind">
        ...
        <!--
        <p>还有{{userInfo.restAward}}元体验金未领取 |</p>
        <router-link :to="{name: 'personTask'}" tag="span"> 去领取</router-link>
        -->
    ...
    methods: {
      login () {
        wlb.ready({
          ...
          other: function () {
            window.location.href = api.host + '/wechat/verify?next=/bbs?source=app'
          }
        })
      }
    }
